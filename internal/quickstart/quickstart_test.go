...
func TestQuickstartDocsReferenceExistingFilesAndCurrentFlags(t *testing.T) {
	...
	requiredQuickstartCommands := []string{
		...
		`nomos.exe policy test --action .\examples\quickstart\actions\allow-readme.json --bundle .\policies\safe-dev-hardened.yaml`,
		`nomos.exe policy test --action .\examples\quickstart\actions\deny-env.json --bundle .\policies\safe-dev-hardened.yaml`,
		`nomos.exe serve -c .\examples\quickstart\config.quickstart.json -p .\policies\safe-dev-hardened.yaml`,
	}
	...
	requiredIntegrationCommands := []string{
		`nomos.exe mcp -c .\examples\quickstart\config.quickstart.json -p .\policies\safe-dev-hardened.yaml`,
		`nomos.exe doctor -c .\examples\quickstart\config.quickstart.json --format json`,
		`nomos.exe serve -c .\examples\quickstart\config.quickstart.json -p .\policies\safe-dev-hardened.yaml`,
	}
...
func TestDockerComposeAndCIArtifactsValidate(t *testing.T) {
	...
	if strings.Join(nomosService.Command, " ") != "nomos serve -c /demo/examples/quickstart/config.quickstart.json -p /demo/policies/safe-dev-hardened.yaml" {
